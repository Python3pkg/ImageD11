...
import tkinter.simpledialog,tkinter.filedialog,os,sys
...
class rubber(Frame):
    ...
    def __init__(self, datafile=None, bkgfile=None, master=None):
        ...
        if datafile==None:
            datafile=tkinter.filedialog.askopenfilename(initialdir=os.getcwd())
        ...
        Button(self.b1,text="Next",command=self.__next__).pack(side=LEFT)
...
#        print image.size
    ...
    def file(self):
        self.datafile=tkinter.filedialog.askopenfilename(initialdir=os.getcwd())
        self.readdata()


    def rdpars(self,parfile=None):
        if parfile==None:
            parfile=tkinter.filedialog.askopenfilename(initialdir=os.getcwd())
    ...
    def rdubis(self,ubifile=None):
        ...
        if ubifile==None:
            ubifile=tkinter.filedialog.askopenfilename(initialdir=os.getcwd())
        self.ubisread = indexing.readubis(ubifile)
        print("Read into self.ubisread",self.ubisread)

    def rdbkg(self, bkgfile=None):
        if bkgfile == None:
            bkgfile=tkinter.filedialog.askopenfilename(initialdir=os.getcwd())
        self.bkgfile=bkgfile
        bkgobj=openimage(bkgfile)
        self.bkg=bkgobj.data.astype(np.int)
        self.data=self.data.astype(np.int)-self.bkg
    ...
    def setmax(self):
        try:
            self.maxi=int(tkinter.simpledialog.askinteger("Maximum data value?","Maximum data value?"))
            self.viewinself()
            stri="<---%f          %f--->"%(self.mini,self.maxi)
            self.keyt.configure(text=stri)
            self.keyt.update()
    ...
    def setmin(self):
        try:
            self.mini=int(tkinter.simpledialog.askinteger("Minimum data value?","Minimum data value?"))
            self.viewinself()
            stri="<---%f          %f--->"%(self.mini,self.maxi)
            self.keyt.configure(text=stri)
            self.keyt.update()
    ...
    def makerockingcurve(self):
        ...
        if xl!=xh and yl!=yh :
            ...
            stem = tkinter.simpledialog.askstring("Filename stem?","Filename stem?",initialvalue=self.datafile[:-8])
            first = tkinter.simpledialog.askinteger("First File to treat","First file to treat")
            last = tkinter.simpledialog.askinteger("Last File to treat", "Last file to treat")
            extn=".edf"
            ...
            d = tkinter.dialog.Dialog(None, {'title': 'Make a rocking curve',
